...
const FiltersSelected = ({ data, setData, changeFilterAttributes }) => {
    ...
    const { listDepartments, listCategories, listColors, listSizes } =
        usePage().props;
    ...
    useEffect(() => {
        ...
        newFiltersSelected.departments = listDepartments.filter(
            (department) => {
        ...
        <>
            ...
            <div className="flex flex-wrap text-xs gap-2.5 mt-4">
                ...
                {filtersSelected.attributes &&
                    filtersSelected.attributes.map((attribute, index) =>
                        attribute.attribute_values.map((value) => (
                            <Badge
                                color="gray"
                                key={"attribute_value" + value.name + index}
                            >
                ...
                {data.price_min && (
                    <Badge color="gray">
                        <span className="mr-2 capitalize">
                            Min {formatCurrency(data.price_min)}
                        </span>
                        <button onClick={() => setData("price_min", "")}>
                            <XMarkIcon className="w-3 h-3" />
                        </button>
                    </Badge>
                )}
                {data.price_max && (
                    <Badge color="gray">
                        <span className="mr-2 capitalize">
                            Max {formatCurrency(data.price_max)}
                        </span>
                        <button onClick={() => setData("price_max", "")}>
                            <XMarkIcon className="w-3 h-3" />
                        </button>
                    </Badge>
                )}
                {data.sortBy && (
                    <Badge color="gray">
                        ...
                        <button onClick={() => setData("sortBy", null)}>
                            <XMarkIcon className="w-3 h-3" />
                        </button>
                    </Badge>
                )}
                {data.offer && (
                    <Badge color="gray">
                        <span className="mr-2 ">Discount {data.offer}%</span>
                        <button onClick={() => setData("offer", null)}>
                            <XMarkIcon className="w-3 h-3" />
                        </button>
                    </Badge>
                )}
